...
if( "$TYPE" == "0" ) then
  set HTML="${BASE_D}/gfs_hd${DATE}/gfs_hd_${TIME}z?time,lat,lon,apcpsfc[0:1:64][0:1:360][0:1:719]"
  if( "$FIELD" == "vor" ) then
  ...
  if( "$PERT" == "00" ) then 
    set HTML="${BASE_E}/gens${DATE}/gec00_${TIME}z?ens,time,lat,lon,apcpsfc[0:1:0][0:1:64][0:1:180][0:1:359]"
    if( "$FIELD" == "vor" ) then
    ...
    set HTML="${BASE_E}/gens${DATE}/gep${PERT}_${TIME}z?ens,time,lat,lon,apcpsfc[0:1:0][0:1:64][0:1:180][0:1:359]"
    if( "$FIELD" == "vor" ) then
...
\rm tmpin/addprecip.$EXT
if("$TYPE" == "0" )then
  set AVFILE=GFS/det_addprecip.in
else
  set AVFILE=GFS/ens_addprecip.in
endif
...
if( "$FIELD" == "vor" ) then
   ...
   if( "$HEMI" == "NH" ) then
      ...
      sed -e "s/EXT/${EXT}/;s@HTML@${HTML}@;s@GFSDET@${DIROUT2}@" $AVFILE > tmpin/addprecip.$EXT
      set FFF=ff_trs_pos.addvor_addmslp_add925winds_add10mwinds
   else
      set DIROUT2=${GFSDIR}/${SUBDIR}/SH/${DIROUT}
      sed -e "s/EXT/${EXT}/;s/pos/neg/;s@HTML@${HTML}@;s@GFSDET@${DIROUT2}@" $AVFILE > tmpin/addprecip.$EXT
      set FFF=ff_trs_neg.addvor_addmslp_add925winds_add10mwinds
   endif
...
$track -w "${HTML}" -f $EXT < tmpin/addprecip.$EXT
mv outdat/ff_trs.${EXT}_addfld ${DIROUT2}/${FFF}_addprecip
