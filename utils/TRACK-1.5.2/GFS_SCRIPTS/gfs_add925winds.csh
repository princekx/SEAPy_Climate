...
if( "$TYPE" == "0" ) then
  set HTML="${BASE_D}/gfs_hd${DATE}/gfs_hd_${TIME}z?time,lev[3:1:3],lat,lon,ugrdprs[0:1:64][3:1:3][0:1:360][0:1:719],vgrdprs[0:1:64][3:1:3][0:1:360][0:1:719]"
  if( "$FIELD" == "vor" ) then
  ...
  if( "$PERT" == "00" ) then 
    set HTML="${BASE_E}/gens${DATE}/gec00_${TIME}z?ens,time,lev[3:1:3],lat,lon,ugrdprs[0:1:0][0:1:64][3:1:3][0:1:180][0:1:359],vgrdprs[0:1:0][0:1:64][3:1:3][0:1:180][0:1:359]"
    if( "$FIELD" == "vor" ) then
    ...
    set HTML="${BASE_E}/gens${DATE}/gep${PERT}_${TIME}z?ens,time,lev[3:1:3],lat,lon,ugrdprs[0:1:0][0:1:64][3:1:3][0:1:180][0:1:359],vgrdprs[0:1:0][0:1:64][3:1:3][0:1:180][0:1:359]"
    if( "$FIELD" == "vor" ) then
...
\rm tmpin/comp925winds.$EXT
\rm tmpin/add925winds.$EXT
if("$TYPE" == "0" )then
  set CWINDS=GFS/det_comp925winds.in
  set AVFILE=GFS/det_add925winds.in
else
  set CWINDS=GFS/ens_comp925winds.in
  set AVFILE=GFS/ens_add925winds.in
endif


if( "$FIELD" == "vor" ) then

   sed -e "s/EXT/${EXT}/" $CWINDS > tmpin/comp925winds.$EXT

   if( "$HEMI" == "NH" ) then
      set DIROUT2=${GFSDIR}/${SUBDIR}/NH/${DIROUT}
      sed -e "s/EXT/${EXT}/;s@GFSDET@${DIROUT2}@" $AVFILE > tmpin/add925winds.$EXT
      set FFF=ff_trs_pos.addvor_addmslp
   else
      set DIROUT2=${GFSDIR}/${SUBDIR}/SH/${DIROUT}
      sed -e "s/EXT/${EXT}/;s/pos/neg/;s@GFSDET@${DIROUT2}@" $AVFILE > tmpin/add925winds.$EXT
      set FFF=ff_trs_neg.addvor_addmslp
   endif
...
$track -w "${HTML}" -f $EXT < tmpin/comp925winds.$EXT

$track -f $EXT < tmpin/add925winds.$EXT
mv outdat/ff_trs.${EXT}_addfld ${DIROUT2}/${FFF}_add925winds
