...
set PERT=$4      # perturbation, 00 for control and deterministic
...
if( "$FIELD" == "vor" ) then
   ...
   if( "$HEMI" == "NH" ) then
      sed -e "s/EXT/${EXT}/;s/SGN/+/;s/NH/${HEMI}/;s/GFSDET/${DIROUT}/" $AVFILE > tmpin/addvor.$EXT
      set DIROUT2=${HOMEDIR}/GFS/NH/${DIROUT}
      set FFF=ff_trs_pos
      gunzip ${DIROUT2}/ff_trs_pos.gz
   else
      sed -e "s/EXT/${EXT}/;s/pos/neg/;s/SGN/-/;s/NH/${HEMI}/;s/GFSDET/${DIROUT}/" $AVFILE > tmpin/addvor.$EXT
      set DIROUT2=${HOMEDIR}/GFS/SH/${DIROUT}
      set FFF=ff_trs_neg
