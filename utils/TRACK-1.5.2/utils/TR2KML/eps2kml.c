...
void write_kml_header(FILE * , long int , int );
...
void process_clusters(FILE * , int * , int , int , int * );
...
int main(int argc, char **argv)
{
   ...
   int *nmnum=NULL, *nmnom=NULL;

   float alat, alng;
   float alatt=0, alngt=0;
   ...
   if(argc != 3){
      printf("Usage: eps2kml [Date] [Hemisphere: NH=0; SH=1]\n\n");
      exit(1);
   } 

   sscanf(argv[1], "%ld", &datei);
   sscanf(argv[2], "%d", &hemi);

   sprintf(lsstr, "./%ld/MATCH", datei);
   ...
   entry = readdir( dir );
   while(entry != NULL){
     if( strncmp( entry->d_name, "trmatch", 7 ) == 0 && ! (strstr(entry->d_name, "mean"))){
        ++nfil; 

        if(nfil == 1){
           nmnum = (int *)malloc_initl(sizeof(int));
           mem_er((nmnum  == NULL) ? 0 : 1, sizeof(int));
           sscanf(entry->d_name + 15, "%d", nmnum); 
        }
        else{
           nmnum = (int *)realloc_n(nmnum, nfil*sizeof(int));
           mem_er((nmnum  == NULL) ? 0 : 1, nfil*sizeof(int));
           sscanf(entry->d_name + 15, "%d", nmnum + nfil -1);
        }
     }
     else if( strncmp( entry->d_name, "trnomatch", 9 ) == 0) {
        ++nnom;

        if(nnom == 1){
           nmnom = (int *)malloc_initl(sizeof(int));
           mem_er((nmnom  == NULL) ? 0 : 1, sizeof(int));
           sscanf(entry->d_name + 17, "%d", nmnom);
        }
        else{
           nmnom = (int *)realloc_n(nmnom, nnom*sizeof(int));
           mem_er((nmnom  == NULL) ? 0 : 1, nnom*sizeof(int));
           sscanf(entry->d_name + 17, "%d", nmnom + nnom -1);
        }
     }
     entry = readdir( dir );
   }
...
/* partition clusters according to mean track properties */
   ...
   for(i=0; i < nfil; i++){
      iclstr = *(nmnum + i);
      if(iclstr < 10) {
         sprintf(trfil, "trmatch_cntl_tr000%d_mean", iclstr);
         sprintf(trfil2, "trmatch_cntl_tr000%d", iclstr);
      }
      else if(iclstr < 100) {
         sprintf(trfil, "trmatch_cntl_tr00%d_mean", iclstr);
         sprintf(trfil2, "trmatch_cntl_tr00%d", iclstr);
      }
      else if(iclstr < 1000) {
         sprintf(trfil, "trmatch_cntl_tr0%d_mean", iclstr);
         sprintf(trfil2, "trmatch_cntl_tr0%d", iclstr);
      }
      else {
      ...
      if(alltr->trid != 0) {
         printf("****ERROR****, first track in file is not analysis. \n\n");
         exit(1);
      }
...
/* determine genesis start */
      ...
      if(alltr->trid == 0) 
         while((alltr+1)->trpt->time != ctim) {++ngtr; ctim = new_time(ctim, TIMINT);}
      else
...
/* write matched clusters and cluster means */
   ...
   process_clusters(fkmlw, iclass, 1, nfil, nmnum);
...
/* matched storms of limited mobility. */
   ...
   process_clusters(fkmlw, iclass, 0, nfil, nmnum);
...
/* non matched storms */
   ...
   for(i=0; i < nnom; i++){
      ...
      iclstr = *(nmnom + i);
      if(iclstr < 10) {
         sprintf(trfil2, "trnomatch_ens000%d", iclstr);
      }
      else if(iclstr < 100) {
         sprintf(trfil2, "trnomatch_ens00%d", iclstr);
      }
      else if(iclstr < 1000) {
         sprintf(trfil2, "trnomatch_ens0%d", iclstr);
      }
      else {
...
void write_kml_header(FILE *fkmlw, long int date, int hemi)
{
   ...
   if(!hemi) fprintf(fkmlw, "\t<name>NH_eps_%ld</name>\n", date);
   else fprintf(fkmlw, "\t<name>SH_eps_%ld</name>\n", date);
   fprintf(fkmlw, "\t<open>0</open>\n");
   fprintf(fkmlw, "\t<visibility>1</visibility>\n");
/* line styles */
   ...
   fprintf(fkmlw, "\t</Style>\n");
   fprintf(fkmlw, "\t<Style id=\"anln\">\n");
   fprintf(fkmlw, "\t\t<LineStyle><color>aa000000</color><width>3</width></LineStyle>\n");
   fprintf(fkmlw, "\t</Style>\n");
/* end of styles */

   if(!hemi){
      fprintf(fkmlw, "\t<description><![CDATA[<p>Northern Hemisphere Tracks</p>");
      fprintf(fkmlw, "<p>Blue lines - Ensemble members</p><p>Red lines - Control</p><p>Green lines - Ensemble mean</p><p>Black lines - Analysis</p>]]></description>\n");
      ...
      fprintf(fkmlw, "\t<description><![CDATA[<p>Southern Hemisphere Tracks</p>");
      fprintf(fkmlw, "<p>Blue lines - Ensemble members</p><p>Red lines - Control</p><p>Green lines - Ensemble mean</p><p>Black lines - Analysis</p>]]></description>\n");
...
/*   fprintf(fkmlw, "\t\t\t<Style><ListStyle><listItemType>checkHideChildren</listItemType></ListStyle></Style>\n");*/
   ...
   fprintf(fkmlw, "\t\t\t\t<visibility>0</visibility>\n");
   fprintf(fkmlw, "\t\t\t\t<open>0</open>\n");
   fprintf(fkmlw, "\t\t\t\t<name>EPS members</name>\n"); 
/* write eps members */
/*   fprintf(fkmlw, "\t\t\t\t\t<Placemark>\n"); 
...
*/
   ...
   if((clstr + 1)->trid == 1){
      for(i=2; i < trcount; i++){
         ...
         fprintf(fkmlw, "\t\t\t\t\t<visibility>0</visibility>\n");
         fprintf(fkmlw, "\t\t\t\t\t<open>0</open>\n");
         fprintf(fkmlw, "\t\t\t\t\t<name>EPS member %d</name>\n", trr->trid - 1);
	 fprintf(fkmlw, "\t\t\t\t\t<Placemark>\n");
	 fprintf(fkmlw, "\t\t\t\t\t<styleUrl>#epsln</styleUrl>\n");
	 fprintf(fkmlw, "\t\t\t\t\t<name>Track</name>\n"); 
         write_kml_track(fkmlw, trr, 0);
	 fprintf(fkmlw, "\t\t\t\t\t</Placemark>\n");
	 fprintf(fkmlw, "\t\t\t\t\t</Folder>\n");
      }
...
/* write control */
   if((clstr + 1)->trid == 1){
      ...
      fprintf(fkmlw, "\t\t\t\t<visibility>0</visibility>\n");
      fprintf(fkmlw, "\t\t\t\t<open>0</open>\n");
      fprintf(fkmlw, "\t\t\t\t<name>Control</name>\n");   
      fprintf(fkmlw, "\t\t\t\t<Placemark>\n");
      fprintf(fkmlw, "\t\t\t\t\t<name>Track</name>\n");
...
/* write eps mean */
   ...
   if(mtr){
      ...
      fprintf(fkmlw, "\t\t\t\t<visibility>0</visibility>\n");
      fprintf(fkmlw, "\t\t\t\t<open>0</open>\n");
      fprintf(fkmlw, "\t\t\t\t<name>EPS Mean</name>\n");
      fprintf(fkmlw, "\t\t\t\t<Placemark>\n");
      fprintf(fkmlw, "\t\t\t\t\t<name>Track</name>\n");
...
/* write analysis */
   ...
   fprintf(fkmlw, "\t\t\t\t<visibility>0</visibility>\n");
   fprintf(fkmlw, "\t\t\t\t<open>0</open>\n");
   fprintf(fkmlw, "\t\t\t\t<name>Analysis</name>\n");
   fprintf(fkmlw, "\t\t\t\t<Placemark>\n");
   fprintf(fkmlw, "\t\t\t\t\t<name>Track</name>\n");
   fprintf(fkmlw, "\t\t\t\t\t<styleUrl>#anln</styleUrl>\n");
...
void process_clusters(FILE *fkmlw, int *iclass, int type, int nfil, int *nmnum)
{
   int i;
   int ifr=0;
   int tr_count=0, mtr_count=0;
   ...
   for(i=0; i < nfil; i++){
      if(*(iclass + i) == type){
         iclstr = *(nmnum + i);
         if(iclstr < 10) sprintf(trfil, "trmatch_cntl_tr000%d", iclstr);
         else if(iclstr < 100) sprintf(trfil, "trmatch_cntl_tr00%d", iclstr);
         else if(iclstr < 1000) sprintf(trfil, "trmatch_cntl_tr0%d", iclstr);
