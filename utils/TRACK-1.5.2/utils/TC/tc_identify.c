...
double anomaly(float * , double * , int , int , int , int );
...
int main(void )
{
    ...
    if(freg){
       ireg = 1;
       fscanf(freg, "%f %f", &(reg.lng1), &(reg.lng2));
       fscanf(freg, "%f %f", &(reg.lat1), &(reg.lat2));
       fscanf(freg, "%ld %ld", &(reg.tim1), &(reg.tim2));
       printf("****INFORMATION****, using region information from region.dat file   \r\n"
              "                     region is (%f, %f) longitude, (%f, %f) latitude,\r\n"
              "                     (%ld, %ld) time.                                \n\n",
              reg.lng1, reg.lng2, reg.lat1, reg.lat2, reg.tim1, reg.tim2);
       fclose(freg);
    }
    else {
    ...
    if(getchar() == 'y'){
       ...
       if(!ilm){
          ...
          printf("Use default latitude ranges or choose, input '0' for default or '1' for choose.\n\n");
	  scanf("%d", &icgen);
	  if(icgen < 0 || icgen > 1){
	  ...
	  if(icgen){
             ...
             printf("****WARNING****, the genesis filtering will be depenent on the land-sea mask.\n\n");
       }
    }
...
/* check if tilts are considered */
             ...
             if(ifcnt){
                ...
                ifncntp = (int *)calloc(irnf, sizeof(int));
                mem_er((ifncntp == NULL) ? 0 : 1, irnf * sizeof(int));
...
/* Are fluxes required? */
             ...
             if(getchar() == 'y'){
	        if(igsmp_typ){
		   printf("****ERROR****, flux calculations not currently possible for rotated lat-lng grids.\n\n");
		   exit(1);
		}
                iflx = 1;
             ...
             if(!iflx){
                ...
                if(getchar() == 'y'){
		   if(igsmp_typ){
		      printf("****ERROR****, advection calculation not currently possible for rotated lat-lng grids.\n\n");
		      exit(1);
		   }
                   iadv = 1;
...
/* scaling of advection term assumes we are working with gradients of geopotential in units of m^2 s^(-2) */
           ...
           if(getchar() == 'y'){
	      if(igsmp_typ){
	         printf("****ERROR****, this set of calculations is not currently available for rotated lat-lng grids.\n\n");
		 exit(1);
	      }
              iaddf = 1;
       ...
       if(ianom || ifanom || ike_cb_anom){
	     ...
	     printf("What uniform distance from center is required in degrees? Note a value greater than the \r\n"
	            "sampled region will be reset to the maximum area.                                       \n\n");
	     scanf("%f", &anom_rad);
	  }
       }
...
*/
       ...
       if(ianom || ifanom){
	     ...
	     nxy2 = irnr / 2;
	     if(anom_rad >= *(slat + irnr - 1)) {anom_rad = *(slat + irnr - 1); iranom = nxy2;}
	     else {
                for(i=0; i < nxy2; i++){
                    if(*(slat + nxy2 + i) > anom_rad) {anom_rad = *(slat + nxy2 + i - 1); iranom = i - 1; break;}	
		}     
	     }
       ...
       ncount = (double **)calloc(irnf, sizeof(double *));
       mem_er((ncount == NULL) ? 0 : 1, irnf * sizeof(double));
       for(i=0; i < irnf; i++){
           *(ncount + i) = (double *)calloc(irdim, sizeof(double));
           mem_er((*(ncount + i) == NULL) ? 0 : 1, irdim * sizeof(double));
...
/* calculate data block size */
       ...
       if(ireg){
         printf("Do you want the choice of field extraction and averaging to be subject to the regional filter, '0' for no and '1' for yes.\n\n");
         scanf("%d", &iregc);
         if(iregc < 0 || iregc > 1){
            printf("****ERROR****, incorrect specifier for region selection.\n\n");
            exit(1);
         }
    ...
    printf("Do you want to output raw data for alternative processing, '1' for yes or '0' for no.  \r\n"
           "At the moment this is only for the primary output and is useful for median calculation.\n\n");
    scanf("%d", &irwout);
    
    if(irwout < 0 || irwout > 1){
       printf("****ERROR****, incorrect option input.\n\n");
       exit(1);
    }
    
    if(irwout){
       frwout = fopen("rawout.dat", "w");
       if(!frwout){
          printf("***ERROR***, unable to open file %s for 'r'\n\n", filin);
          exit(1);
       }
       
       fprintf(frwout, "%6d %6d %6d %4d %2d %2d\n", ntrout, irnth, irnr, irnf, iext, imxmn);
       fwrite(slng, irnth*sizeof(float), 1, frwout);
       fprintf(frwout, "\n");
       fwrite(slat, irnr*sizeof(float), 1, frwout);
       fprintf(frwout, "\n");
    ...
    if(nff){
       ...
       if(itv == 'y'){
          ...
          printf("****WARNING****, levels should be ordered lower to upper.\n\n");
          ...
          printf("Specify which pair of additonal fields to use for warm core test, \r\n"
                 ...
                 "Note, a more refined warm/cold core test is possible.             \n\n");
          scanf("%d %d", &if1, &if2);
          ...
          if(getchar() == 'y') {
            ifs = 1;
            if(nvert) exit(1);
            printf("Do you want to check for decreasing/increasing vorticity (warm/cold core) with height between \r\n"
                   "specified pairs of additional fields between those chosen for warm core, 'y' or 'n'           \n\n");
            scanf("\n");
            if(getchar() == 'y'){
               ifs2 = 1;
               printf("Use all additional fields between levels chosen for warm/cold core check or select levels, '0' for select and '1' for all.\n\n");
               scanf("%d", &ifsel);
               if(!ifsel){
               ...
               wcsc = (float *)calloc(nlv-1, sizeof(float));
               mem_er((wcsc == NULL) ? 0 : 1, (nlv-1)*sizeof(float));
               printf("Use only warm core checks, 'y' or 'n'?\n\n");
               scanf("\n");
               if(getchar() == 'y'){
                  for(i=0; i < nlv - 1; i++) *(wcsc + i) = 1.0;
               }
               else{
                  printf("Input scaling for warm (1.0) or cold(-1.0) core checks.\n\n");
                  for(i=0; i < nlv - 1; i++){
                      printf("What is the scaling for level pair %d? ", i+1);
                      scanf("%f", wcsc + i);
                      if(fabs(fabs(*(wcsc + i)) - 1.0) > 1.0e-5){
                         printf("****ERROR****, scaling must be + or - unity.\n\n");
                         exit(1); 
                      }
                  }
               }
...
/* open file for writing rotation angle and speeds */
    ...
    for(i=itrst; i < itren; i++){
       ...
       if(igg){
	     ...
	     if(icgen){
	        if(fp->yf < glat1 || fp->yf > glat2) igen = 0;
	     }
	     else {
       ...
       if(ireg && !iregc){
          ...
          inreg = inregion(&reg, lngm, latm, tim);
       ...
       for(j=0; j < atr->num; j++){
           ...
           if(ifs){
		     ...
		     if(stl2 > ADD_CHECK) {iv = -1; break;}
                     if(ifsel && *(wcsc + k) * (stl1 - stl2) < 0.0){iv = 0; break;}
                     stl1 = stl2;
                     nn += 3;
                 }
              }
		 ...
		 if(stl1 > ADD_CHECK) iv = -1;
                    ...
                    for(k=1; k < nlp; k++){
                        ...
                        stl2 = fp->add_fld[nn+2];
		        if(stl2 > ADD_CHECK) {iv = -1; break;}
                        if(ifsel && *(wcsc + k - 1) * (stl1 - stl2) < 0.0){iv = 0; break;}
                        stl1 = stl2;
                        nn += 3;
                    }
		 ...
		 if(stl1 > ADD_CHECK) iv = -1;
                    ...
                    for(k=1; k < nlv; k++){
                        ...
                        stl2 = fp->add_fld[nn];
			if(stl2 > ADD_CHECK) {iv = -1; break;}
                        if(*(wcsc + k - 1) * (stl1 - stl2) < 0.0){iv = 0; break;}
                        stl1 = stl2;
                    }
		 }
	      ...
	      if(iv < 0) continue; 
           ...
           if(itv == 'y') {
              if(!if1) {
	         if(fp->add_fld[iff2] > ADD_CHECK) fdif = 0.0;
                 else fdif = fp->zf - fp->add_fld[iff2];
	      }
              else {
	         if(fp->add_fld[iff1] > ADD_CHECK || fp->add_fld[iff2] > ADD_CHECK) fdif = 0.0;	
                 else fdif = fp->add_fld[iff1] - fp->add_fld[iff2];
           ...
           if(str >= thresh && fdif >= fth && iv == nlp){
              ...
              if(irad){
                 if(!iext){
                    if(imxmn){
                       if(iregc) {
                         if(inregion(&reg, fp->xf, fp->yf, (atr->time) ? fp->time : (long int)(fp->fr_id)) && str > fmxmn) {fmxmn = str; ifld = j;}
                       }
                       else {
                    ...
                    if(j < atr->num - 1){
                       ...
                       if(imxmn){
                          if(iregc){
                             if(inregion(&reg, fp->xf, fp->yf, (atr->time) ? fp->time : (long int)(fp->fr_id)) && tend > fmxmn) {fmxmn = tend; ifld = j;}
                          }
                          else {
                   ...
                   if(iregc) {
                      if(inregion(&reg, fp->xf, fp->yf, (atr->time) ? fp->time : (long int)(fp->fr_id)) && fdif > fmxmn) {fmxmn = fdif; ifld = j;}
                   }
                   else {
                   ...
                   if(atr->time){
                     if(fp->time == ctime) {
                        ifld = j;
                        if(iregc){
                          if(!inregion(&reg, fp->xf, fp->yf, fp->time)) ifld = -1;
                        }
                     }
                   }
                   else{
                     if((long int)(fp->fr_id) == ctime) {
                        ifld = j;
                        if(iregc) {
                          if(!inregion(&reg, fp->xf, fp->yf, (long int)(fp->fr_id))) ifld = -1;
                        }
                     }
       ...
       if(iext == 4) {
          ifld = 0;
          if(iregc &&
             !inregion(&reg, atr->trpt->xf, atr->trpt->yf, (atr->time) ? atr->trpt->time : (long int)(atr->trpt->fr_id))) ifld = -1;
       }
...
/*          fprintf(ftim, "%5d\n", (atr->trpt + ifld)->fr_id); */
          if(atr->time) fprintf(ftim, "%5d %ld\n", ifld + 1, (atr->trpt + ifld)->time);
	  else fprintf(ftim, "%5d %5d\n", ifld + 1, (atr->trpt + ifld)->fr_id);         
          fprintf(fdout, "%f\n", fdifmax);
             ...
             if(!icomp_order){
                for(j=0; j < irnf; j++){
                   ...
                   if(iaddf){
                      ...
                      fscanf(fadd, "%*c");
                      if(ifanom) anomaly(sadd, coslat, irnr, irnth, iranom, igsmp_typ);
...
/* write radial distributions */
		   ...
		   if(irwout){
		     fwrite(sradf1, irdim*sizeof(float), 1, frwout);
                     fprintf(frwout, "\n");
		   }
             ...
             if(iwind && icomp_order){
                ...
                fpc = atr->trpt + ifld;
		if(ifdir){
		   if(*(fpc->add_fld + ifdirp) > ADD_CHECK) continue;  
	           xx = *(fpc->add_fld + ifdirp) * FP_PI;
                   yy = FP_PI2 - *(fpc->add_fld + ifdirp + 1) * FP_PI;		
		}
		else {
                   xx = fpc->xf * FP_PI;
...
/* determine sampling point at this level */
		      ...
		      setup_rotmatrix(s1, c1, s2, c2, rot);
                   ...
                   if(iflx){
                      if(!iffl){
                         ...
                         if(ifanom) {
                            anomaly(tmp1, coslat, irnr, irnth, iranom, igsmp_typ);
                            anomaly(tmp2, coslat, irnr, irnth, iranom, igsmp_typ);
                         }
                         for(k=0; k < irdim; k++) {
                            uu = *(tmp1 + k);
                            vv = *(tmp2 + k);
                   ...
                   if(ikecb){
                      ...
                      fscanf(fadd, "%*c");
                      if(ike_cb_anom) anomaly(sadd2, coslat, irnr, irnth, iranom, igsmp_typ);
                   }

                   if(ianom){
                      anomaly(sradf1, coslat, irnr, irnth, iranom, igsmp_typ);
                      anomaly(sradf2, coslat, irnr, irnth, iranom, igsmp_typ);
                   }
		       ...
		       if(!igsmp_typ) {
			     ...
			     vv -= pdirl;
                   ...
                   if(idevazm){
                      ...
                      aravg = anomaly(sadd, coslat, irnr, irnth, irr, igsmp_typ);
    ...
    if(irwout){
       ...
       fseeko(frwout, (off_t)0, SEEK_SET);
       fprintf(frwout, "%6d %6d %6d %4d %2d %2d\n", ntrout, irnth, irnr, irnf, iext, imxmn);
       fclose(frwout);
    }
    ...
    if(irad){
       ...
       printf("Use computed number values for averaging or read from file, '0' or '1'\n\n");
       scanf("%d", &iatyp);
       if(iatyp < 0 || iatyp > 1) {
          printf("****ERROR****, incorrect input for averaging type.\n\n");
	  exit(1);
       }
       if(iatyp){
          printf("What numbers file is required, must be in binary format.\n\n");
	  scanf("%s", numsfil);
	  
	  fnum = fopen(numsfil, "r");
          if(!fnum){
             printf("****INFORMATION****, can't open file %s for read.\n\n", numsfil);
	     exit(1);
          }
	  
	  fgets(buff, MAXCHR, fnum);
	  sscanf(buff, "%d %d %d", &nmnth, &nmnr, &nmlev);
	  if(nmnth != irnth || nmnr != irnr || nmlev != irnf){
	     printf("****ERROR****, incompatable grid for nums file %s\n", numsfil);
	     exit(1);
	  ...
	  for(i=0; i < nmnth; i++){
	      fscanf(fnum, "%f", &tvar);
	      ++ia;
	      if(ia == 10){fgets(buff, 100, fnum); ia = 0;}
	      if(fabs(tvar - *(slng + i)) > GTOL) {
	         printf("****ERROR****, grids do not match.\n\n");
		 exit(1);
	      }
	  }
	  if(ia) fgets(buff, 100, fnum);
	  ...
	  for(i=0; i < nmlev; i++){
	     fgets(buff, MAXCHR, fnum);
	     fread(fldt, irdim*sizeof(float), 1, fnum);
	     fscanf(fnum, "%*c");
	     for(j=0; j < irdim; j++) *(*(ncount + i) + j) = *(fldt + j);
	  }
...
/*                for(j=0; j < irdim; j++) *(sradf1 + j) = (float)(*(savg3[i] + j));
                *(vke + i) = anomaly(sradf1, coslat, irnr, irnth, irr, igsmp_typ);               */   
...
/* write to file */
       ...
       write_fields("nums.dat", ncount, slng, slat, irnth, irnr, irnf);
       ...
       if(iwind){
          if(!igsmp_typ){
             ...
             write_fields("reg_avg_uu.dat", aavg1, slng, slat, irnth, irnr, irnf);
             write_fields("reg_avg_vv.dat", aavg2, slng, slat, irnth, irnr, irnf);
             if(istd && icomp_order){
                write_fields("reg_std_uu.dat", astd1, slng, slat, irnth, irnr, irnf);
                write_fields("reg_std_vv.dat", astd2, slng, slat, irnth, irnr, irnf);
             }	  
	  }
          if(iflx){
       ...
       if(getchar() == 'y'){
          ...
          write_fields_netcdf("nums.nc", ncount, slng, slat, levdat, irnth, irnr, irnf, iext, imxmn, iwind, -1, ctime);
	     ...
	     if(!igsmp_typ){
                ...
                write_fields_netcdf("reg_avg_uu.nc", aavg1, slng, slat, levdat, irnth, irnr, irnf, iext, imxmn, iwind, 1, ctime);
                write_fields_netcdf("reg_avg_vv.nc", aavg2, slng, slat, levdat, irnth, irnr, irnf, iext, imxmn, iwind, 2, ctime);
                if(istd && icomp_order){
                   write_fields_netcdf("reg_std_uu.nc", astd1, slng, slat, levdat, irnth, irnr, irnf, iext, imxmn, iwind, 1, ctime);
                   write_fields_netcdf("reg_std_vv.nc", astd2, slng, slat, levdat, irnth, irnr, irnf, iext, imxmn, iwind, 2, ctime);
                }	     
	     }
             if(iflx){
          ...
          if(icomp_order){
             ...
             if(iflx){
                for(i=0; i < irnf; i++) {
                    ...
                    free(linteg1[i]); free(linteg2[i]); free(linteg3[i]);
                }
                free(savg1);
                free(savg2);
                free(savg3);
...
double anomaly(float *sradf, double *coslat, int irnr, int irnth, int iran, int igsmp_typ)
{
     int i, j;
     int irdim=0;
     int i1=0, i2=0, nxy2=0;
...
int inregion(REG *reg, float lngm, float latm, long int tim)
{

    int inreg=0;

    if(reg->lng1 > reg->lng2){
      if((reg->lat2 - latm) * (latm - reg->lat1) >= 0.0 && 
        ((reg->lng2 - lngm) * lngm >= 0.0 || (360.0 - lngm) * (lngm - reg->lng1) >= 0.0)) inreg = 1;
    }

    else if((reg->lng2 - lngm) * (lngm - reg->lng1) >= 0.0 && (reg->lat2 - latm) * (latm - reg->lat1) >= 0.0) inreg = 1;

    if(inreg && tim >= reg->tim1 && tim <= reg->tim2) inreg = 1;
    else inreg = 0;
