...
void rotated_cyln_grid_assign(GRID * , CNTRY * , float , float , int , int , int , int , int , int , int );

void rotated_cyln_grid(int ptp)

{
   ...
   static char *prname[] = {"Rotated Plate Caree", "Rotated Equirectangular", "Rotated Equal Area",
                            "Rotated Urmaev", "Rotated Mercator",
                            "Rotated Stereographic", "Rotated Miller"};
   
   static PROJ proj={NULL, NULL};
   
   ppm = &proj;
   
			    
   printf("Is data already on a rotated cyclindrical grid, '0' for no or '1' for yes.\n\n");
   ...
   if(ptp < 0){
      ...
      printf("which rotated cylindrical projection is required,   \r\n"
             "available projections are:                          \r\n\n"
	     "Rotated Plate Caree          input '0'              \r\n"
             "Rotated Equirectangular      input '1'              \r\n"
             "Rotated Equal Area           input '2'              \r\n"
             "Rotated Urmaev               input '3'              \r\n"
	     "Rotated Mercator             input '4'              \r\n"
	     "Rotated Stereographic        input '5'              \r\n"
	     "Rotated Miller               input '6'              \n\n"
      );   
      ...
      proj.prj1 = proj_assign(gt->prty);
      proj.prj2 = proj_assign(prty);
   ...
   printf("what is the rotation for the projection, input a (lat, long) pair.\n\n");
   ...
   strncpy(gt->prty_nm, prname[prty], MXPRCH);
   strncpy(gr1->prty_nm, prname[prty], MXPRCH);
...
/* find region extent in projected space */
   ...
   rotated_cyln_grid_assign(gr1, cm1, gr1->alat, gr1->alng, gr1->ox1u, gr1->ox2u, gr1->oy1u, gr1->oy2u, prty, iaz, ift);
...
void rotated_cyln_grid_assign(GRID *ggr, CNTRY *cmm, float alat, float alng, int xng1, int xng2, int ylt1, int ylt2, int prty, int iaz, int ift)
      ...
      if(!iaz){
         ...
         for(i=ylt1-1; i < ylt2; i++){
             ...
             for(j=xng1-1; j < xng2; j++){
                 ...
                 if(rotated_cyln(&yy, &xx, alat, alng, 1, &ift)){
