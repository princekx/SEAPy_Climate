...
void fel_mge(ADPT **add, ZONE **zone, struct frame_objs *fo, struct track_ind *tind, float *dmax, float *phimax, int frame_num, int fi_count, int npar)

{
    ...
    for(k=1; k < frame_num-1; k++){
        ...
        for(i=0; i < track_num-1; i++){
            ...
            devii = (i == 0) ? (*(trwsp + i) = devn(fo, tpsi[0], tpsi[1], tpsi[2], phim)) : *(trwsp + i);
...
/* check feature point pairs are within requisit distance */
                 ...
                 if(izz){
                    ...
                    ndmax1 = ub_disp(fo+k, fo+k+1, tpsi[1], tpsj[2], zone[im2], dm);
                    ndmax2 = ub_disp(fo+k, fo+k+1, tpsj[1], tpsi[2], zone[im2], dm);

                 }

                 dispij = disp(fo+k, fo+k+1, tpsi[1], tpsj[2], ndmax1);

                 dispji = disp(fo+k, fo+k+1, tpsj[1], tpsi[2], ndmax2);   
...
/* if feature points are within the requisite distance compute the exchange gain */
                   ...
                   if(dispij <= ndmax1 && dispji <= ndmax2){
                      ...
                      if(iadpt){
                        ...
                        devij = devn(fo, tpsi[0], tpsi[1], tpsj[2], phim);

                        if(noph)

                           nphi1 = phi(dispij, disp(fo+k-1, fo+k, tpsi[0], tpsi[1], dm), add[im1]);

                        devji = devn(fo, tpsj[0], tpsj[1], tpsi[2], phim);

                        if(noph)

                           nphi2 = phi(dispji, disp(fo+k-1, fo+k, tpsj[0], tpsj[1], dm), add[im1]);
...
/* exchange points if required */
        ...
        if(gijmax > 0.0){
           ...
           else if(tpsi[2]->feature_id != -1 && tpsj[2]->feature_id == -1){
           ...
           *tpsi[2] = tps;
           ...
           if(k+1 < frame_num-1){
             ...
             if(izz){
                 ...
                 ndmax1 = ub_disp(fo+k+1, fo+k+2, tpsi[2], tpsi[2]+1, zone[im2], dm);
                 ndmax2 = ub_disp(fo+k+1, fo+k+2, tpsj[2], tpsj[2]+1, zone[im2], dm);

              }

             dis1 = disp(fo+k+1, fo+k+2, tpsi[2], tpsi[2]+1, ndmax1);
             dis2 = disp(fo+k+1, fo+k+2, tpsj[2], tpsj[2]+1, ndmax2);
 
             if(dis1 > ndmax1) track_fail(fo, tind, iex, k, frame_num, 'f');
             if(dis2 > ndmax2) track_fail(fo, tind, jex, k, frame_num, 'f');
    ...
    printf("forward iteration %d completed, cost function= %f\n", fi_count, totgij);

    if(ex_fl == 1){ 

       bi_fl = 1;
       fel_mge(add, zone, fo, tind, dmax, phimax, frame_num, fi_count, npar);
