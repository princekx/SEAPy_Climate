...
/* evaluate the sample statistics for a quadratic kernal function */


double variance_quad(double , struct dpt * , struct dpt * , double * , float * , int , int , double );
double quad_density(struct dpt * , struct dpt * , double );

double eval_quad_stat(double *plt, double **den, struct dpt *st, struct dpt *dt, float *wght, int dtn, int ptnum, int im, int ht, int ind, int ms)

{
    ...
    double nn=0.0, tsm=0., ts;

    double cn=0.0, con=0., cnn, dd, cd;
    ...
    if(!wght){
       ...
       cn = (nn > TOLWT) ? 3.0 / (2.0 * FPI * nn) : 0.0;

    }

    if(ht < 3){

      if(*plt < SMMIN){ tsm = -1.0; con = 1.0;}
       ...
       *d[0] = 0.0;
       ...
       if(ht < 3){
         ...
         for(j=0; j < dtn; j++){
            ...
            *d[0] += (dd = wt * (dtmp = quad_density(spt, dtt, tsm)));
            dent += dtmp;
         ...
         for(j=0; j < dtn; j++){
            ...
            *d[0] += (dd = wt * (dtmp = cnn * quad_density(spt, dtt, tsm)));
            dent += dtmp;
       ...
       *d[0] *= con;
       ...
       if(d[1]) {
          *d[1] = (*d[0] > DTOL) ? *d[1] * con / *d[0] : 0.0;
          ...
          if(iv) *d[2] = sqrt(variance_quad(mean, spt, dt, plt, wght, dtn, ht, nn));
       }
       if(d[3]){
...
double variance_quad(double mean, struct dpt *spt, struct dpt *dt, double *plt, float *wght, int dtn, int ht, double nn)

{
    ...
    if(ht < 3){
      ...
      for(j=0; j < dtn; j++){
         ...
         den += (dd = wt * quad_density(spt, dtt, tsm));
      ...
      for(j=0; j < dtn; j++){
         ...
         den += (dd = wt * cnn * quad_density(spt, dtt, tsm));
