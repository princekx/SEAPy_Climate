...
/* evaluate the sample statistics for a constant kernal function */


double variance_const(double , struct dpt * , struct dpt * , double * , float * ,int , int , double );
double const_density(struct dpt * , struct dpt * , double );

double eval_const_stat(double *plt, double **den, struct dpt *st, struct dpt *dt, float *wght, int dtn, int ptnum, int im, int ht, int ind, int ms)
       ...
       *d[0] = 0.0;
       ...
       if(ht < 3){
         ...
         for(j=0; j < dtn; j++){
            ...
            *d[0] += (dd = wt * (dtmp = const_density(spt, dtt, tsm)));
            dent += dtmp;
         ...
         for(j=0; j < dtn; j++){ 
            ...
            *d[0] += (dd = wt * (dtmp = cnn * const_density(spt, dtt, tsm)));
            dent += dtmp;
       ...
       *d[0] *= con;
       ...
       if(d[1]) {
          *d[1] = (*d[0] > DTOL) ? *d[1] * con / *d[0] : 0.0;
          ...
          if(iv) *d[2] = sqrt(variance_const(mean, spt, dt, plt, wght, dtn, ht, nn));
       }
       if(d[3]){
...
double variance_const(double mean, struct dpt *spt, struct dpt *dt, double *plt, float *wght, int dtn, int ht, double nn)

{
    ...
    if(ht < 3){
      ...
      for(j=0; j < dtn; j++){
         ...
         den += (dd = wt * const_density(spt, dtt, tsm));
      ...
      for(j=0; j < dtn; j++){
         ...
         den += (dd = wt * cnn * const_density(spt, dtt, tsm));
