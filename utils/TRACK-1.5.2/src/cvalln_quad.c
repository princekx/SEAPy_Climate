...
/* calculate cross-validation function for a quadratic kernal function */

double quad_density(struct dpt * , struct dpt * , double );

double cvalln_quad(struct dpt *dt, int dtn, double sm, double *plt, float *wght, int im, int ht, int ty, int ms)
   ...
   for(i=0; i < dtn; i++) {
       ...
       if(ht < 3) {
          ...
          for(j=0; j < dtn; j++){
              ...
              fd += (ftd = wt * quad_density(dt1, dt2, tsm));
          ...
          for(j=0; j < dtn; j++){
              ...
              fd += (ftd = wt * cnn * quad_density(dt1, dt2, tsm));
