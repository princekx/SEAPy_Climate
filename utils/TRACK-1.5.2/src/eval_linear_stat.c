...
double variance_linear(double , struct dpt * , struct dpt * , double * , float * ,int , int , double );
double linear_density(struct dpt * , struct dpt * , double );

double eval_linear_stat(double *plt, double **den, struct dpt *st, struct dpt *dt, float *wght, int dtn, int ptnum, int im, int ht, int ind, int ms)
       ...
       *d[0] = 0.0;
       ...
       if(ht < 3){
         ...
         for(j=0; j < dtn; j++){
            ...
            *d[0] += (dd = wt * (dtmp = linear_density(spt, dtt, tsm)));
            dent += dtmp;
         ...
         for(j=0; j < dtn; j++){ 
            ...
            *d[0] += (dd = wt * (dtmp = cnn * linear_density(spt, dtt, tsm)));
            dent += dtmp;
       ...
       *d[0] *= con;
       ...
       if(d[1]) {
          ...
          *d[1] = (*d[0] > DTOL) ? *d[1] * con / *d[0] : 0.0;
          ...
          if(iv) *d[2] = sqrt(variance_linear(mean, spt, dt, plt, wght, dtn, ht, nn));
       }
       if(d[3]){
...
double variance_linear(double mean, struct dpt *spt, struct dpt *dt, double *plt, float *wght, int dtn, int ht, double nn)

{
    ...
    if(ht < 3){
      ...
      for(j=0; j < dtn; j++){
         ...
         den += (dd = wt * linear_density(spt, dtt, tsm));
      ...
      for(j=0; j < dtn; j++){
         ...
         den += (dd = wt * cnn * linear_density(spt, dtt, tsm));
