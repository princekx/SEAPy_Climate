...
class KFfilter:
    ...
    # source: https://github.com/tmiyachi/mcclimate/blob/master/kf_filter.py
    ...
    def igfilter(self, fmin=None, fmax=None, kmin=-15, kmax=-1, hmin=12, hmax=90, n=1):
        ...
        if n <= 0 or n % 1 != 0:
            print()
            "n must be n>=1 integer. for n=0 EIG you must use eig0filter method."
            sys.exit()
    ...
    def eig0filter(self, fmin=None, fmax=0.55, kmin=0, kmax=15, hmin=12, hmax=50):
        ...
        if kmin < 0:
            print()
            "kmin must be positive. if k < 0, this mode is MRG"
            sys.exit()
    ...
    def mrgfilter(self, fmin=None, fmax=None, kmin=-10, kmax=-1, hmin=8, hmax=90):
        ...
        if kmax > 0:
            print()
            "kmax must be negative. if k > 0, this mode is the same as n=0 EIG"
            sys.exit()
    ...
    def mjofilter(self, fmin=0.01, fmax=0.05, kmin=1, kmax=9):
        """MJO filter. 
